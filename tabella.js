import { SETTABELLA, GETTABELLA, removeMarker, cleanHTML, AddMAP, GETMAPPA, SETDATI, map, zoom, render } from './progetto.js';
    ...
    load: function () {
      GETTABELLA()
        .then((cachedData) => {
          ...
          // Aggiorna la mappa con i dati caricati
          originale.forEach((row) => {
            if (row.length === 4 && row[1] !== "N/A" && row[2] !== "N/A" && row[3] !== "N/A") {
              const luogo = row[0];
              const titolo = luogo;
              AddMAP(luogo, titolo, row[1], row[2], row[3], GETMAPPA, SETDATI, map, zoom);
            }
          });
